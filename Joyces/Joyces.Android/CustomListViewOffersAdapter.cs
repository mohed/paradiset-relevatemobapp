...
namespace Joyces.Droid
{
    class CustomListViewOffersAdapter : BaseAdapter<Offer>
    {
        ...
        private View buildOfferView(int position, View convertView, ViewGroup parent)
        {
            View row = convertView;
            if (row == null)
        ...
        private View buildStampCardView(int position, View convertView, ViewGroup parent)
        {
            View row = convertView;
            if (row == null)
            {
            ...
            txtValue.Text = String.Format("{0:0}%", mItem[position].value);
            ImageService.Instance.LoadUrl(mItem[position].imageUrl).Into(imgOfferImage);
            return row;
        }
    }    
